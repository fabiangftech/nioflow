...
The codebase already calls `when()` / `match()` "forks" (`DefaultNioFlowForkTest`, "fork predicates run on the boss", `Condition`/`Branch`/`Cases`). Two different things cannot both be "the fork".
...
**Unit tests** — new class `DefaultNioFlowDetachedForkTest` (the `when`/`match` coverage stays in `DefaultNioFlowForkTest`), extending `EngineTestSupport`:
