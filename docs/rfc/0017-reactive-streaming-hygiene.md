...
1. **Streaming discipline** — make the bounded Flux path the easy one and the unbounded one loud, closing RFC 0004's own loose end.
2. **Blocking fast-path** — do not install a `CountDownLatch` when the Mono is already resolved (a cache hit, a `Mono.just`).
...
| Benchmark | Must |
| --- | --- |
| new `syncMono` bench through `handleMono` | faster (no latch); async Mono unchanged |
| `monoOverhead` | not regress |
