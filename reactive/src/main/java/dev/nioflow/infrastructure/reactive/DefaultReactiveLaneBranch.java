...
class DefaultReactiveLaneBranch<T> extends DefaultReactiveLane<T> implements ReactiveLaneBranch<T> {
    ...
    DefaultReactiveLaneBranch(LaneBranch<T> branch, Duration budget, boolean preferAsync) {
        super(branch, budget, preferAsync);
        this.branch = branch;
    }
    ...
    public ReactiveLane<T> otherwise(UnaryOperator<Lane<T>> lane) {
        return new DefaultReactiveLane<>(
                branch.otherwise(Lanes.budgeted(lane, budget, preferAsync)), budget, preferAsync);
    }
}
