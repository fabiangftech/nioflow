...
- Per-call allocation discipline: the context map stays null until a contextual stage's first put (or a caller-handed map), `call()` returns the raw result future — bookkeeping (drain slot, metrics, handlers) runs inside `Execution` BEFORE completion, so no dependent `whenComplete` future — and the boss→worker→boss dispatch is a manual two-closure handoff, not `CompletableFuture` composition (CF stays where its semantics pay: per-attempt `orTimeout`, fan-out `allOf`). A boss rejection mid-flight fails the future, never hangs it. ~727 B/op on a plain sealed chain, 248 B/op boss-inlined.
