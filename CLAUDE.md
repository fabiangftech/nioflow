...
- `Lane<T>` exposes only step-building operations (`handle`/`background`/`adapt`/`filter` and nested `when`/`match` via `LaneCondition`/`LaneBranch`/`LaneCases`) — no `just`, no `execute`, no `close`. The type going into and out of a lane is `T` (an `adapt` inside a lane must return to `T`, enforced by the `UnaryOperator` signature).
