...
- **`core/facade`** — the public API contracts. `NioFlow<T>` is the user-facing fluent interface (`just`, `handle`, `submit`, `batch`, `fanOut`, `when`/`match` forks, `filter`, `onErrorResume`, `seal`, `join`, ...). `NioEngine` is the engine contract behind it — untyped on purpose so `adapt` can hand out a differently-typed `NioFlow` view over the same running engine. `NioFlowGateway<T, R>` is the request/response bridge: one `call` per injected value, resolved with that value's own result (`CompletableFuture`), for request-driven callers. `Resilience`, `NioFlowMetrics`, `NioFlowTracer` are pluggable policy/observability contracts.
