...
- **`infrastructure`** — optional adapters over `compileOnly` dependencies: `OpenTelemetryMetrics` (metrics SPI → otel histograms/counters/gauge; only loads if the consumer brings `opentelemetry-api`) and `Resilience4jStages` (circuit breaker / bulkhead decorators for stage functions; breaker-open and bulkhead-full failures flow to `recover()` like any stage failure, each `Retry` attempt passes the decorators, bulkhead waits park the virtual worker; only loads if the consumer brings resilience4j).
